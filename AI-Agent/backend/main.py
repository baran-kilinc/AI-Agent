...
from backend.rag import ChromaRetriever, build_rag_context, build_rag_system_instruction
...
async def chat(payload: dict):
    ...
    if retriever.is_ready:
        try:
            retrieved_docs = retriever.query(message)
            context = build_rag_context(retrieved_docs, retriever.max_context_chars)
            if context:
                messages.append(
                    {
                        "role": "system",
                        "content": build_rag_system_instruction(context),
                    }
                )
        except Exception as exc:
            logger.exception("RAG context build failed, continuing without context: %s", exc)
